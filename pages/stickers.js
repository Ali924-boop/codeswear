...
export async function getServerSideProps(context) {
  ...
  let products = await Productdb.find({ category: 'stickers' });
  let stickers = {};
  ...
  for (let item of products) {
    if (item.title in stickers) {
      if (item.availableQty > 0) {
        ...
        if (item.color) {
          if (Array.isArray(item.color)) {
            item.color.forEach((c) => {
              ...
              if (!stickers[item.title].color.includes(cLower)) {
                stickers[item.title].color.push(cLower);
              }
            });
        ...
        if (item.size) {
          if (typeof item.size === 'string') {
            ...
            if (!stickers[item.title].size.includes(sizeVal)) {
              stickers[item.title].size.push(sizeVal);
            }
        ...
        // Images: add unique
        if (item.img && !stickers[item.title].images.includes(item.img)) {
          stickers[item.title].images.push(item.img);
        }
      }
    } else {
      ...
      if (item.color) {
        if (Array.isArray(item.color)) {
          stickers[item.title].color = item.color.map((c) => c.toLowerCase());
        } else if (typeof item.color === 'string') {
      ...
      if (item.size) {
        if (typeof item.size === 'string') {
          stickers[item.title].size = [item.size.trim().toUpperCase()];
        } else if (Array.isArray(item.size)) {
      ...
      stickers[item.title].images = item.img ? [item.img] : [];
    }
  }

  // Optional: Debug output to check final stickers structure
  // console.log("stickers aggregated:", stickers);
