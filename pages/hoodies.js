...
export async function getServerSideProps(context) {
  ...
  let products = await Productdb.find({ category: 'hoodies' });
  let hoods = {};
  ...
  for (let item of products) {
    if (item.title in hoods) {
      if (item.availableQty > 0) {
        ...
        if (item.color) {
          if (Array.isArray(item.color)) {
            item.color.forEach((c) => {
              ...
              if (!hoods[item.title].color.includes(cLower)) {
                hoods[item.title].color.push(cLower);
              }
            });
        ...
        if (item.size) {
          if (typeof item.size === 'string') {
            ...
            if (!hoods[item.title].size.includes(sizeVal)) {
              hoods[item.title].size.push(sizeVal);
            }
        ...
        // Images: add unique
        if (item.img && !hoods[item.title].images.includes(item.img)) {
          hoods[item.title].images.push(item.img);
        }
      }
    } else {
      ...
      if (item.color) {
        if (Array.isArray(item.color)) {
          hoods[item.title].color = item.color.map((c) => c.toLowerCase());
        } else if (typeof item.color === 'string') {
          hoods[item.title].color = [item.color.toLowerCase()];
        }
      }
      ...
      if (item.size) {
        if (typeof item.size === 'string') {
          hoods[item.title].size = [item.size.trim().toUpperCase()];
        } else if (Array.isArray(item.size)) {
      ...
      hoods[item.title].images = item.img ? [item.img] : [];
    }
  }

  // Optional: Debug output to check final hoods structure
  // console.log("hoods aggregated:", hoods);
